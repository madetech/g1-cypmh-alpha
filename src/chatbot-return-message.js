...
const getNextChatState = async (currentChatState, response) => {
  ...
  let matchedStateId = chatStates[currentChatState].returnOptionIds
    .find(id => {
        return response.includes(chatStates[id].captureStrings)
      })
  ...
  if (matchedStateId !== undefined || chatStates[matchedStateId].outgoingMessage !==null) {
